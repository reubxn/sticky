...
- **Orchestrated Capture System**: Local Taste Engine also exposes `/api/captures/ingest/orchestrated`, which wraps capture ingestion with one Claude transcript-to-decision summarizer agent. Campaign-compression image generation does not use a second prompt-writing agent; it always uses the canonical fixed Campaign Compression Sphere prompt plus the source screenshot.
- **Sphere Image Proxy**: Cloudflare Worker exposes optional `POST /image` for provider-secret-backed sphere image generation (`OPENAI_API_KEY`, optional `IMAGE_MODEL`). The route must fail clearly when not configured; generated image assets still need durable local persistence before they count as complete Sphere evidence.
- **TTS Fallbacks**: Cloudflare Worker `POST /tts` can use ElevenLabs or OpenAI speech (`TTS_PROVIDER=openai`). The macOS TTS client also falls back to the local system voice if ElevenLabs credentials or calls fail.
...
| `taste-fingerprint-studio/app/api/captures/ingest/orchestrated/route.ts`      | Wraps capture ingestion with orchestrated Claude-backed decision summary and canonical sphere prompt routing.                        |
...
| `taste-fingerprint-studio/lib/orchestration/decision-package-orchestrator.ts` | Orchestration module for summary-agent flow, canonical image prompt routing, and provenance enrichment.                              |
...
| `taste-fingerprint-studio/lib/orchestration/image-client.ts`                  | Shared Worker image-route client for campaign-compression sphere generation.                                                         |
| `worker/src/index.ts`                                                         | Cloudflare Worker proxy for Claude chat, ElevenLabs TTS, AssemblyAI token generation, and optional sphere image generation.          |
